...
class TableHeap {
 ...
 explicit TableHeap(BufferPoolManager *buffer_pool_manager, Schema *schema, Txn *txn, LogManager *log_manager,
      ...
      : buffer_pool_manager_(buffer_pool_manager),
        ...
        log_manager_(log_manager),
        lock_manager_(lock_manager) {
  // Step 1: Allocate a new page to serve as the root/first page of the table heap.
  page_id_t root_page_id;
  auto* page = (TablePage*)buffer_pool_manager_->NewPage(root_page_id);
  if (page == nullptr) {
   throw std::runtime_error("Failed to allocate a new page for the table heap.");
  }

  // Step 2: Since this is a new table, initialize the table page.
  // Note: Assuming TablePage has an Init method. This will need to match your actual implementation.
  page->Init(root_page_id, PAGE_SIZE,  log_manager_, txn);

  // Ensure the page is written back to disk and is no longer pinned in buffer.
  buffer_pool_manager_->UnpinPage(root_page_id, true);

  // Record the root page id as the first page of the table.
  first_page_id_ = root_page_id;

  // Further initialization can go here. For example, setting up metadata, indices, etc., as necessary.
 };
